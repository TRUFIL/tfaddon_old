...
frappe.provide("tfaddon.back_office");
frappe.provide("tfaddon.back_office.job_order");
...
frappe.ui.form.on('Job Order', {
	...
	print_without_amount: function(frm) {
		tfaddon.back_office.job_order.set_print_hide(frm.doc);
	},
...
tfaddon.back_office.JobOrderController = erpnext.selling.SellingController.extend({
	setup: function(doc) {
	...
	refresh: function(doc, dt, dn) {
				...
				this.frm.add_custom_button(__('Sales Order'),
					function() {
						erpnext.utils.map_current_doc({
							method: "erpnext.selling.doctype.sales_order.sales_order.make_job_order",
		...
		tfaddon.back_office.job_order.set_print_hide(doc, dt, dn);
	},
	...
	close_job_order: function(doc){
		this.update_status("Closed")
	},

	reopen_job_order : function() {
	...
	update_status: function(status) {
		...
		frappe.call({
			method:"tfaddon.back_office.doctype.job_order.job_order.update_job_order_status",
...
$.extend(cur_frm.cscript, new tfaddon.back_office.JobOrderController({frm: cur_frm}));
...
tfaddon.back_office.job_order.set_print_hide = function(doc, cdt, cdn){
	var dn_fields = frappe.meta.docfield_map['Job Order'];
	var dn_item_fields = frappe.meta.docfield_map['Job Order Item'];
